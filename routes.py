...
router = APIRouter()
## POST
@router.post("/", response_description="Dodaj nowa transakcje", status_code=status.HTTP_201_CREATED, response_model=Transakcje)
...
## GET ALL
@router.get("/", response_description="Wszystkie transakcje", response_model=List[Transakcje])
def wszystkie_transakcje(request: Request):
    transakcje = list(request.app.database["pythonproject"].find(limit=100))
    return transakcje

## GET by ID
@router.get("/{id}", response_description="Znajdz transakcje po id", response_model=Transakcje)
